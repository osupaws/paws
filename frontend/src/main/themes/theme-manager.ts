...
function parseTheme(themeDir: string): Theme | null {
  ...
  if (!fs.existsSync(manifestPath)) {
    log(`Skipping directory: No theme.json found in ${path.basename(themeDir)}`);
    return null;
  }
  ...
  try {
    ...
    if (!manifest.id || !manifest.name || !manifest.base || !manifest.file) {
      log(`Skipping theme in ${path.basename(themeDir)}: Manifest is missing required fields (id, name, base, file).`);
      return null;
    }
    
    // Construct the file path relative to the theme's directory
    manifest.file = `themes/${path.basename(themeDir)}/${manifest.file}`;
